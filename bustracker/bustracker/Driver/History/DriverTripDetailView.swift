...
struct DriverTripDetailView: View {
    ...
    private var eventsList: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if events.isEmpty {
                    Text("No scans were recorded on this trip.")
                        .font(.subheadline)
                        .foregroundStyle(Color.appSecondary)
                        .padding(.top, 8)
                } else {
    ...
    private var summaryCard: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(trip.studentNames.isEmpty ? "No scans recorded" : trip.studentNames.joined(separator: ", "))
                .font(.headline)
                .foregroundStyle(.white)
            HStack(spacing: 14) {
    ...
    private func load() async {
        ...
        errorMessage = nil
        do {
            async let pathFetch = SupabaseDriverHistoryService.fetchTripPath(tripID: trip.tripID)
            async let eventsFetch = SupabaseDriverHistoryService.fetchTripEvents(tripID: trip.tripID)
